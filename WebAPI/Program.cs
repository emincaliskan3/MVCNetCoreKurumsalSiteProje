...
builder.Services.AddControllers().AddJsonOptions(x => x.JsonSerializerOptions.ReferenceHandler = ReferenceHandler.IgnoreCycles); // include iþleminde aldýðýmýz hatayý çözmek için 
...
builder.Services.AddSwaggerGen(c =>
{
    ...
    c.AddSecurityDefinition("Bearer", new OpenApiSecurityScheme
    {
        ...
        Description = "Lütfen JWT Bearer token bilginizi girinizç",
        Name = "Authorization",
...
// kendi yazdýðýmýz servisleri uygulamada kullanabilmek için ekliyoruz aksi taktirde System.InvalidOperationException: Unable to resolve service for type 'Service.IService`1[Entities.User]' while attempting to activate 'WebAPI.Controllers.AuthController'... hatasý veriyor
...
builder.Services.AddAuthentication
    (JwtBearerDefaults.AuthenticationScheme).AddJwtBearer(opt =>
    {
        opt.TokenValidationParameters = new Microsoft.IdentityModel.Tokens.TokenValidationParameters
        {
            ...
            ValidAudience = builder.Configuration["Token:Audience"], // token veren saðlayýcý
